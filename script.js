...
const LOCAL_STORAGE_KEY = "downloadedGames";
const DOWNLOADED_QUERY_PARAM = "downloaded";

function getGameId(game) {
  return game.download_link || game.title;
}

function encodeDownloadedQueryValue(gameId) {
  try {
    const utf8Bytes = encodeURIComponent(gameId).replace(/%([0-9A-F]{2})/g, (_, hex) =>
      String.fromCharCode(parseInt(hex, 16)),
    );
    return btoa(utf8Bytes).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
  } catch (e) {
    console.error("Error encoding downloaded game for URL:", e);
    return encodeURIComponent(gameId);
  }
}

function decodeDownloadedQueryValue(value) {
  try {
    const normalized = value.replace(/-/g, "+").replace(/_/g, "/");
    const padded = normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "=");
    const decoded = atob(padded);
    const percentEncoded = Array.from(decoded, (char) => `%${char.charCodeAt(0).toString(16).padStart(2, "0")}`).join(
      "",
    );
    ...
    try {
      ...
      console.error("Error decoding downloaded game from URL:", decodeError);
      return value;
    }
  }
}

function parseDownloadedGamesFromUrl() {
  const params = new URLSearchParams(window.location.search);
  return params
    .getAll(DOWNLOADED_QUERY_PARAM)
    .flatMap((value) => value.split(","))
    .map((value) => value.trim())
    .filter(Boolean)
    .map(decodeDownloadedQueryValue);
}

function writeDownloadedGames(downloadedGames) {
  const serialized = JSON.stringify(downloadedGames);
...
function getDownloadedGames() {
  try {
    ...
    return {
      ...
      ...(sessionStored ? JSON.parse(sessionStored) : {}),
    };
  } catch (e) {
    console.error("Error reading downloaded games cache:", e);
    return {};
  }
}
...
function saveDownloadedGame(game) {
  try {
    ...
    writeDownloadedGames(downloadedGames);
    console.log(`✅ Saved download state for: ${game.title}`);
    return true;
  } catch (e) {
    console.error("Error saving downloaded game cache:", e);
    return false;
  }
}
...
function cacheDownloadedGamesFromQuery(games) {
  const queriedGameIds = parseDownloadedGamesFromUrl();
  if (queriedGameIds.length === 0) return;

  const gamesById = new Map(games.map((game) => [getGameId(game), game]));
  const downloadedGames = getDownloadedGames();
  let hasChanges = false;

  queriedGameIds.forEach((gameId) => {
    if (downloadedGames[gameId]) return;

    const game = gamesById.get(gameId);
    downloadedGames[gameId] = game
      ? {
          title: game.title,
          download_link: game.download_link,
          platform: game.platform,
  ...
  if (hasChanges) {
    writeDownloadedGames(downloadedGames);
    console.log(`🔁 Restored ${queriedGameIds.length} downloaded game(s) from URL query`);
  }
}

function syncDownloadedGameToUrl(game) {
  const gameId = getGameId(game);
  const encodedGameId = encodeDownloadedQueryValue(gameId);
  ...
  if (!existingIds.has(encodedGameId)) {
    url.searchParams.append(DOWNLOADED_QUERY_PARAM, encodedGameId);
...
function sortGamesByPopularity(games) {
  return games.sort((a, b) => {
    const aIndex = POPULAR_GAMES.findIndex(
      (title) =>
        a.title.toLowerCase().includes(title.toLowerCase()) || title.toLowerCase().includes(a.title.toLowerCase()),
    );
    const bIndex = POPULAR_GAMES.findIndex(
      (title) =>
        b.title.toLowerCase().includes(title.toLowerCase()) || title.toLowerCase().includes(b.title.toLowerCase()),
    );
...
async function loadGames(retryCount = 0, fileName = "games.json") {
  ...
  try {
    ...
    const validGames = games.filter(
      (game) => game && game.download_link !== null && game.title && game.platform && game.thumbnail,
    );
    ...
    cacheDownloadedGamesFromQuery(sortedGames);
    ...
    if (downloadedCount > 0) {
      console.log(`📥 You have ${downloadedCount} downloaded game(s) cached`);
    }
  } catch (error) {
...
function isPopularGame(gameTitle) {
  return POPULAR_GAMES.some(
    (title) =>
      gameTitle.toLowerCase().includes(title.toLowerCase()) || title.toLowerCase().includes(gameTitle.toLowerCase()),
  );
}
  ...
  pageGames.forEach((game) => {
      ...
      ? `
            ...
            <div class="game-info">
                ...
                <div class="card-actions">
                    <span class="platform-chip">Platform: ${game.platform}</span>
    ...
    setTimeout(() => {
      ...
      btn.addEventListener("click", function (e) {
        // Save download state to cache and URL query
        saveDownloadedGame(game);
        syncDownloadedGameToUrl(game);
...
function filterGames() {
  ...
  let filtered = allGames.filter((game) => {
    const matchesSearch =
      game.title.toLowerCase().includes(searchQuery) || game.platform.toLowerCase().includes(searchQuery);
